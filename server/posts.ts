...
function rowToPost(row: PostRow): Post {
  ...
  if (row.summary !== null) post.summary = row.summary;
...
function str(
  ...
  if (trimmed.length > max) {
    throw new ValidationError(
      `${field} exceeds ${max} characters (got ${trimmed.length})`,
    );
  }
...
function normalizeTs(input: unknown): string {
  ...
  if (typeof input !== "string") {
    throw new ValidationError("ts must be an ISO-8601 string");
  }
  const parsed = new Date(input);
  if (Number.isNaN(parsed.getTime())) {
    throw new ValidationError(`ts is not a valid date: ${input}`);
  }
...
export function validate(input: unknown): ValidatedPost {
  ...
  return {
    ...
    summary: str(raw.summary, "summary", MAX_SUMMARY),
...
function writePost(db: Db, p: ValidatedPost): PostWriteResult {
  ...
  const existingId = p.dedupe_key
    ? findDedupeTarget(db, p.dedupe_key, now)
    : null;
  ...
  if (existingId) {
    ...
    db.prepare(
      `UPDATE posts
			    SET ts = ?, source = ?, kind = ?, title = ?, summary = ?, body = ?, tags = ?,
    ...
    db.prepare(
      `INSERT INTO posts
			   (id, ts, source, kind, title, summary, body, tags, project, repo, meta, dedupe_key, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      id,
      p.ts,
  ...
  // getPost, so a deduped update carries the read state it already had -- mutating
  // a post in place (a build going #running -> #passed) shouldn't silently mark it
  // unread, nor clear a marker the reader set. A brand-new post comes back unseen.
  return { post: getPost(db, id)!, deduped: existingId !== null };
}
...
function toFtsQuery(q: string): string | null {
  ...
  return tokens.map((t, i) => (i === tokens.length - 1 ? `"${t}"*` : `"${t}"`))
    .join(" AND ");
}
...
export function buildWhere(
  ...
  if (excludedSources.length) {
    where.push(
      `p.source NOT IN (${excludedSources.map(() => "?").join(", ")})`,
    );
  ...
  if (excludedTags.length) {
    where.push(
      `NOT EXISTS (SELECT 1 FROM post_tags t WHERE t.post_id = p.id AND t.tag IN (${
        excludedTags
          .map(() => "?")
          .join(", ")
      }))`,
...
export function queryPosts(db: Db, query: PostQuery): PostPage {
  ...
  return {
    posts,
    next_cursor: hasMore && posts.length > 0
...
export function countPosts(db: Db): number {
  const row = db.prepare("SELECT COUNT(*) AS n FROM posts")
...
export function sweepRetention(db: Db, days: number): number {
  ...
  if (deleted > 0) {
    console.log(
      `[retention] deleted ${deleted} post(s) older than ${days} days`,
    );
  }
  return deleted;
}
