...
import { countPosts, insertPost, queryPosts, ValidationError } from './posts.ts';
...
Deno.test('rejects posts missing required fields', () => {
	const d = db();
	assertThrows(() => insertPost(d, { title: 'no source' }), ValidationError, 'source is required');
	assertThrows(() => insertPost(d, { source: 'cli' }), ValidationError, 'title is required');
	assertThrows(() => insertPost(d, { source: 'cli', title: '   ' }), ValidationError, 'title is required');
	assertThrows(() => insertPost(d, { source: 'cli', title: 'x', ts: 'nonsense' }), ValidationError);
...
Deno.test('ts is normalized to UTC so lexicographic ordering is chronological', () => {
	...
	insertPost(d, { source: 'a', title: 'earlier', ts: '2026-07-13T23:00:00+02:00' }); // 21:00Z
...
Deno.test('meta.project and meta.repo are promoted to filterable columns', () => {
	...
	insertPost(d, {
		source: 'claude-code',
		title: 'edited a file',
		meta: { project: 'dev-stream', repo: 'phin-tech/dev-stream', branch: 'main' }
	});

	assertEquals(queryPosts(d, { project: ['dev-stream'] }).posts.length, 1);
	assertEquals(queryPosts(d, { repo: ['phin-tech/dev-stream'] }).posts.length, 1);
...
Deno.test('dedupe_key posts afresh outside the window', () => {
	const d = db();
	const first = insertPost(d, { source: 'ci', title: 'nightly failed', dedupe_key: 'nightly' });
	...
	d.prepare('UPDATE posts SET created_at = ? WHERE id = ?').run(stale, first.post.id);

	const second = insertPost(d, { source: 'ci', title: 'nightly failed', dedupe_key: 'nightly' });
...
Deno.test('dedupe replaces tags rather than accumulating them', () => {
	const d = db();
	insertPost(d, { source: 'ci', title: 'run', dedupe_key: 'k', tags: ['running'] });
	insertPost(d, { source: 'ci', title: 'run', dedupe_key: 'k', tags: ['passed'] });
...
Deno.test('filters combine with AND, and multiple tags require all of them', () => {
	const d = db();
	insertPost(d, { source: 'ci', kind: 'alert', title: 'a', tags: ['deploy', 'failed'] });
	...
	assertEquals(queryPosts(d, { source: ['ci'], tag: ['failed'] }).posts.length, 1);
});
...
Deno.test('since/until bound the range inclusively', () => {
	...
	assertEquals(queryPosts(d, { since: '2026-06-01T00:00:00Z' }).posts.map((p) => p.title), [
		'new',
		'mid'
	]);
	assertEquals(
		queryPosts(d, { since: '2026-02-01T00:00:00Z', until: '2026-07-01T00:00:00Z' }).posts.map(
			(p) => p.title
		),
		['mid']
	);
...
Deno.test('full-text search covers title, body and tags', () => {
	const d = db();
	insertPost(d, { source: 'a', title: 'Deployed to production', body: 'took 4 minutes' });
...
Deno.test('search treats FTS5 operators as literal text rather than syntax', () => {
	...
	for (const q of ['"', 'foo"bar', 'AND', 'NOT OR', 'a*', 'col:value', '-x', '(((', '^']) {
		const page = queryPosts(d, { q });
		assertEquals(page.posts.length, 0, `expected no match for ${JSON.stringify(q)}`);
	}
...
Deno.test('keyset pagination walks every post exactly once', () => {
	...
	for (let i = 0; i < 7; i++) insertPost(d, { source: 'a', title: `post ${i}`, ts });
...
Deno.test('pagination is stable when new posts arrive mid-scroll', () => {
	const d = db();
	for (let i = 0; i < 4; i++) {
		insertPost(d, { source: 'a', title: `old ${i}`, ts: `2026-07-13T0${i}:00:00Z` });
	}
	...
	insertPost(d, { source: 'a', title: 'brand new', ts: '2026-07-13T09:00:00Z' });
...
Deno.test('an invalid cursor is a client error, not a crash', () => {
	const d = db();
	assertThrows(() => queryPosts(d, { cursor: 'not-base64!!' }), ValidationError, 'invalid cursor');
});
...
Deno.test('posting registers the source', () => {
	...
	const rows = d.prepare('SELECT slug FROM sources').all() as unknown as { slug: string }[];
