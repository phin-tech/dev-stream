...
const filterShape = {
  source: z.array(z.string()).optional().describe(
    'Origin slugs, e.g. ["claude-code", "ci"].',
  ),
  project: z.array(z.string()).optional(),
  repo: z.array(z.string()).optional().describe(
    'e.g. ["phin-tech/dev-stream"].',
  ),
  kind: z.array(z.string()).optional().describe(
    "event | note | alert | pr | issue",
  ),
  tag: z.array(z.string()).optional().describe(
    "A post must carry ALL of these tags.",
  ),
  since: z.string().optional().describe(
    "ISO-8601. Inclusive lower bound on event time.",
  ),
  until: z.string().optional().describe(
    "ISO-8601. Inclusive upper bound on event time.",
  ),
};
...
export function createMcpServer(): McpServer {
    ...
    "post_to_timeline",
    {
      ...
      inputSchema: {
        title: z.string().describe(
          "One line, in past tense. This is the card headline.",
        ),
        summary: z.string().optional().describe(
          "Optional plain-text compact-card context.",
        ),
        ...
        source: z
          ...
          .optional()
          .describe(
            'Origin slug. Defaults to "agent" — override to attribute it elsewhere.',
          ),
        dedupe_key: z
          .string()
          .optional()
          .describe(
            "Re-posting the same key within 10 minutes updates that card instead of adding one.",
          ),
      },
    },
    async (args) => {
      ...
      try {
        ...
        return text(
          `Could not post: ${err instanceof Error ? err.message : String(err)}`,
          true,
        );
      }
    },
  ...
  server.registerTool(
    "get_view_posts",
    {
      ...
      description:
        "Runs a saved view by name and returns its posts, newest first.",
      inputSchema: {
    ...
    async ({ view, limit }) => {
      try {
        ...
        const match = views.find((v) =>
          v.name.toLowerCase() === view.toLowerCase()
        );
        if (!match) {
