...
POST OPTIONS
  ...
  --summary <text>      compact-card context
...
async function cmdPost(args: string[]): Promise<void> {
  ...
  if (!title) {
    fail("nothing to post: pass a message, --title, or pipe something in");
  }

  // The body is the piped input, unless the piped input *was* the title.
  const body = flags.body ??
    (stdin.trim() && stdin.trim() !== title ? stdin.trimEnd() : undefined);
...
function report(via: "api" | "spool", count: number, detail?: string): void {
  ...
  // But say so, or a user will think their post vanished.
      ...
      `dev-stream isn't running — spooled ${count} ${noun} to ${detail}`,
    );}
}
...
async function cmdTail(args: string[]): Promise<void> {
  ...
  for await (const chunk of res.body) {
    ...
    while ((boundary = buffer.indexOf("\n\n")) !== -1) {
      ...
      console.log(
        `${time}  ${event.post.source.padEnd(12)} ${event.post.title}${
          tags ? "  " + tags : ""
        }`,
      );
    }
...
async function cmdStatus(): Promise<void> {
  ...
  if (!config) {
    console.log(
      "dev-stream is not running. Posts will be spooled until it starts.",
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `dev-stream ${info.version} running on 127.0.0.1:${info.port} (pid ${info.pid})`,
    );
  } catch {
    // The port file outlives an ungraceful exit, so a stale one is expected.
    console.log(
      `~/.dev-stream/port says ${config.port}, but nothing is answering there.`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "init": {
      if (rest[0] !== "claude") {
        fail("only 'dev-stream init claude' is supported");
      }
      ...
      console.log(
        `${result.created ? "Created" : "Updated"} ${result.path}\n` +
          ...
          `Claude Code sessions in ${
            flags.global ? "any project" : "this project"
          } will now post to your timeline.`,
      );
      return;
    }
    case "--version":
